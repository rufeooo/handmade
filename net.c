...
{
	struct sockaddr_in sa = { .sin_family = AF_INET, .sin_addr.s_addr = htonl(ip), .sin_port = htons(port) };
...
int main(int argc, char** argv)
{
	...
	if (argc < 2) // SERVER
	{
		...
		do
		{
			size_t byte_count = recvfrom(s, buffer, sizeof(buffer)-1, 0,
				(struct sockaddr*)&msg_sa, &(socklen_t){sizeof(msg_sa)});
			if (byte_count == -1) {
		...
		*write = 0;
		...
		if (!sendto(s, buffer, write-buffer, 0, (struct sockaddr*)&peer_sa, sizeof(peer_sa))) {
